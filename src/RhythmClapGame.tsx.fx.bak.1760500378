...
const APPROACH_VEL   = -0.40; // 靠近的速度門檻
...
export default function RhythmClapGame(){
  ...
  function sweepTimeoutMisses(){
    ...
    for(let i=0;i<list.length;i++){
      ...
      // 若已超過「容許寬限（GOOD_MS）」還未拍到，直接標記 Miss
  ...
  useEffect(()=>{
    ...
    function detect(){
      ...
      const thr=handScaleRef.current*1.00*clamp(sensitivity,0.6,1.6); // 門檻稍降
      ...
      <div className="col left-col">
        <div className="card">
          ...
          <div style={{marginTop:10,display:"flex",gap:10,flexWrap:"wrap",alignItems:"center"}}>
            <button className="btn" onClick={startCalibration} disabled={phase==="calibrating"||phase==="playing"}>1) 校準（80 BPM）</button>
            <button className="btn" onClick={startGame} disabled={phase!=="ready" && phase!=="finished"}>2) 開始遊戲</button>
            <span className="pill">基準延遲：{baseDelayMs} ms</span>
          ...
          <div style={{marginTop:10, display:"grid", gridTemplateColumns:"repeat(3,minmax(0,1fr))", gap:8}}>
            <div style={{background:"#0f1635",border:"1px dashed #2b3874",borderRadius:12,padding:10}}>
              ...
              <div>倍率：<b>{fmt(sensitivity,2)}×</b>（往左 &lt; 1.0 更容易觸發）</div>
            </div>
            <div style={{background:"#0f1635",border:"1px dashed #2b3874",borderRadius:12,padding:10}}>
              <div><b>🧪 偵測狀態</b></div>
              <div>dist：{fmt(debugRef.current.dist)} px</div>
              <div>vel ：{fmt(debugRef.current.vel,2)} px/frame</div>
              <div>thr ：{fmt(debugRef.current.thr)} px</div>
              <div>state：<b>{debugRef.current.state}</b></div>
            </div>
            <div style={{background:"#0f1635",border:"1px dashed #2b3874",borderRadius:12,padding:10}}>
              <div><b>📊 拍手事件</b></div>
              <div>偵測到拍手：<b>{clapCount}</b> 次</div>
              <div>最後一次評分：<b>{lastVerdict}</b></div>
            </div>
          </div>
      ...
      <div className="col right-col">
        <div className="card">
          ...
          <div className="score-grid">
            ...
            <div className="kpi acc"><div className="v">{acc}%</div><div className="t">綜合分數</div></div>
          </div>
        </div>

        <div className="card" style={{marginTop:16}}>
          <h2>📷 攝影機 + 偵測（畫面可見除錯文字）</h2>
          {/* 只用 canvas 畫面；如要隱藏 video，給 style display:none */}
